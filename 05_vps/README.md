...
ConoHa VPS に Tailscale・Nginx・Certbot を導入し、外部公開の設定をします。

---

## 前提条件

- ConoHa VPS を契約済みであること（1コア・RAM 1GB 推奨）
- OS：Ubuntu Server 26.04 LTS
- [04_services](../04_services/README.md) が完了していること
- ドメインを取得済みであること
- DNS レコードを VPS の IP アドレスに向けていること
...
## 1. VPS の初期設定（ConoHa コンソールで実施）

ConoHa の管理画面からコンソールを開き、以下を実行します。
...
```bash
ssh -i ~/.ssh/id_ed25519_homelab <ユーザー名>@<VPSのTailscale IP>
```
...
> ⚠️ iptables か Nginx stream モジュールか、どちらを使うか要検討。
> 現時点では設定例のみ記載します。

### iptables を使う場合

```bash
# Factorio（34197/UDP）を自宅サーバの Tailscale IP に転送
sudo iptables -t nat -A PREROUTING -p udp --dport 34197 \
  -j DNAT --to-destination <自宅サーバのTailscale IP>:34197
...
stream {
    server {
        listen 34197 udp;
        proxy_pass <自宅サーバのTailscale IP>:34197;
    }
    server {
        listen 7777 udp;
        proxy_pass <自宅サーバのTailscale IP>:7777;
    }
}
```
